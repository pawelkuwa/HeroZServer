...
class Guild extends DBGuild implements JsonSerializable{
    ...
    public function loadGuild(){
        ...
        if($this->pending_dungeon != null && $this->pending_dungeon->isFought)
            ...
            $reward = GuildBattleRewards::find(function($q){ $q->where('character_id',Core::player()->character->id)->where('type',3); });
            if($reward){
                $this->finished_dungeon = GuildDungeonBattle::findFinished($this);
                $this->addBattleReward($reward);
            }
        }
    }
    ...
    private function setDungeonPendingAsFinished(){
        $this->finished_dungeon = $this->pending_dungeon;
