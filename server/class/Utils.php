...
class Utils{
    ...
    public static function getOwnedItemTemplates($player, $type, $quality = null){
        ...
        $owned = $templates[$type] ?? [];
        if($quality !== null && !empty($owned)){
            $typeName = Item::$TYPE[$type];
            $owned = array_values(array_filter($owned, function($identifier) use ($typeName, $quality){
                foreach(ItemsList::$ITEMS[$typeName] as $item){
                    if($item['identifier'] == $identifier && $item['quality'] == $quality) return true;
                }
                return false;
            }));
        }
    ...
    public static function addItemToOwnedTemplates($player, $item){
        ...
        if($player->getLVL() < $sewingMachineReqLevel) return;
        if($item->type > 7) return false;

        $result = \Srv\DB::sql('SELECT `owned_item_templates` FROM `character` WHERE `id` = ?', [$player->character->id])->fetch();
        $templates = json_decode($result['owned_item_templates'] ?? '{}', true) ?? [];

        if(!isset($templates[$item->type])) $templates[$item->type] = [];
        ...
        if(!in_array($item->identifier, $templates[$item->type])){
            ...
            Core::req()->append['owned_item_templates'] = $templates;
            return true;
        }
        return false;
    }
    ...
    public static function addItemToPattern($player, $item){
        ...
        $itemType = Item::$TYPE[$item->type] ?? null;
        if(!$itemType) return;
        ...
        if(empty($itemPatternFiltered)) return;

        $itemData = reset($itemPatternFiltered);
        if(empty($itemData['item_pattern'])) return;

        $patternName = $itemData['item_pattern'];
        $itemPattern = GameSettings::getConstant("item_pattern.{$patternName}", null);
        if(empty($itemPattern)) return;

        $currentPatternValues = json_decode($player->character->current_item_pattern_values ?? '{}', true) ?? [];
        ...
        if(!in_array($item->identifier, $currentPatternValues[$patternName]['collected_items'] ?? [])){
            ...
            $player->character->current_item_pattern_values = json_encode($currentPatternValues);
            \Srv\DB::sql('UPDATE `character` SET `current_item_pattern_values` = ? WHERE `id` = ?', [
                json_encode($currentPatternValues),
                $player->character->id
            ]);

            Core::req()->append['current_item_pattern_values'] = $currentPatternValues;
        }
    }
