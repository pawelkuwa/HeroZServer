...
class claimDungeonQuestRewards{
    ...
    public function __request($player){
        ...
        if($battle->winner == 'a'){
            $player->incrementGoalStat('fight_quests_won_hard');
            $player->updateHerobookProgress(14);
        }
        $player->incrementGoalStat('dungeon_quests_completed');
        $player->updateHerobookProgress(6);
        ...
        if($quest->battle_id != 0) {
            Battle::delete(function($q)use($quest, $battle){ $q->where('id',$battle->id); });
        }
